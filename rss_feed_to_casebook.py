...
def clean_entry(entry_link):  
    ...
    # new code to make make scrape look like a browser session to bypass website urlopen
    # security checks
    req = Request(entry_link, headers={'User-Agent': 'Mozilla/6.0'})
    encoded_data = urlopen(req).read()
    html = encoded_data.decode('utf-8')
    soup = BeautifulSoup(html, "html.parser")
